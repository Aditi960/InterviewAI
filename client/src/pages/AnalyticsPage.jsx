...
export default function AnalyticsPage() {
  ...
  const topicData = data?.topicPerformance?.map(t => ({
    ...
    avg: t.avgScore ?? t.average,
    count: t.count,
    fill: (t.avgScore ?? t.average) < 6 ? '#f87171' : '#00e5ff',
  })) || []

  const radarData = data?.topicPerformance?.map(t => ({
    topic: t.topic || t.name,
    score: t.avgScore ?? t.average,
  })) || []

  const weakTopics = data?.topicPerformance?.filter(t => (t.avgScore ?? t.average) < 6) || []
  ...
  return (
    <div className="p-8">
      ...
      <div className="flex gap-4 mb-8 flex-wrap">
        {[
          { label: 'Total Interviews', value: data?.stats?.totalInterviews ?? data?.totalInterviews ?? 0, color: 'bg-sky-100 text-sky-700' },
          { label: 'Average Score', value: `${data?.stats?.averageScore ?? data?.averageScore ?? 0}/10`, color: 'bg-amber-100 text-amber-700' },
          { label: 'Best Score', value: `${data?.stats?.bestScore ?? data?.bestScore ?? 0}/10`, color: 'bg-emerald-100 text-emerald-700' },
          { label: 'Weak Topics', value: data?.stats?.weakTopicsCount ?? data?.weakTopicsCount ?? 0, color: 'bg-red-100 text-red-600' },
        ].map(s => (
      ...
      {weakTopics.length > 0 && (
        <div className="card border-l-4 border-red-300">
          ...
          <div className="grid grid-cols-3 gap-3">
            {weakTopics.map(t => (
              <div key={t.topic || t.name} className="bg-red-50 border border-red-100 rounded-xl p-3">
                <p className="font-medium text-red-700 text-sm">{t.topic || t.name}</p>
                <p className="text-2xl font-display font-bold text-red-500 mt-1">{t.avgScore ?? t.average}/10</p>
              </div>
            ))}
          </div>
        </div>
