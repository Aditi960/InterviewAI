...
const SessionDetail = ({ id }) => {
  ...
  return (
    <div className="max-w-[800px] mx-auto">
      <button onClick={() => navigate('/history')} className="flex items-center gap-1.5 mb-4 sm:mb-5 min-h-[44px] text-sm" style={{ background: 'none', border: 'none', color: '#64748b', cursor: 'pointer', fontFamily: 'inherit' }}>
        <ArrowLeft size={16} /> Back to History
      </button>

      {/* Header card */}
      <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-6 p-5 sm:p-7 mb-4 sm:mb-5" style={{ background: 'white', borderRadius: 16, boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
        <ScoreRing score={session.score} />
        <div className="flex-1 text-center sm:text-left">
          <h2 className="text-lg sm:text-[22px] font-bold mb-1.5" style={{ fontFamily: 'Syne, sans-serif', color: '#1e293b', margin: '0 0 6px' }}>{session.role}</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 mb-4 sm:mb-5">
        {[
          ...
          <div key={title} className="p-4 sm:p-5" style={{ background: 'white', borderRadius: 16, boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
            <div className="flex items-center gap-2 mb-3 sm:mb-3.5">
              <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: bg }}>
                <Icon size={16} color={color} />
              </div>
              <span className="text-sm font-bold" style={{ fontFamily: 'Syne, sans-serif', color: '#1e293b' }}>{title}</span>
            </div>
            <ul className="m-0 pl-4">
              {(items || []).map((item, i) => (
                <li key={i} className="text-xs sm:text-[13px] mb-1" style={{ color: '#475569', lineHeight: 1.6 }}>{item}</li>
              ))}
            </ul>
          </div>
        ))}
      ...
      {session.topicAnalysis?.length > 0 && (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 p-4 sm:p-6 mb-4 sm:mb-5" style={{ background: 'white', borderRadius: 16, boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
          <div>
            <h3 className="text-sm sm:text-[15px] font-bold mb-3 sm:mb-4" style={{ fontFamily: 'Syne, sans-serif', color: '#1e293b' }}>Topic Scores</h3>
            {session.topicAnalysis.map(t => (
              <div key={t.name} className="mb-3 sm:mb-3.5">
                <div className="flex justify-between mb-1">
                  <span className="text-xs sm:text-[13px] font-medium" style={{ color: '#475569' }}>{t.name}</span>
      ...
      {session.questions?.length > 0 && (
        <div className="p-4 sm:p-6" style={{ background: 'white', borderRadius: 16, boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
          <h3 className="text-sm sm:text-[15px] font-bold mb-4 sm:mb-5" style={{ fontFamily: 'Syne, sans-serif', color: '#1e293b' }}>Question Breakdown</h3>
          {session.questions.map((q, i) => (
            <div key={i} style={{ borderBottom: i < session.questions.length - 1 ? '1px solid #f1f5f9' : 'none', paddingBottom: i < session.questions.length - 1 ? 16 : 0, marginBottom: i < session.questions.length - 1 ? 16 : 0 }}>
              <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-1 sm:gap-3 mb-2">
                <div className="flex gap-2">
                  ...
                  <p className="m-0 text-sm font-semibold leading-relaxed" style={{ color: '#1e293b' }}>{q.question}</p>
                </div>
              ...
              {q.answer && <p className="text-xs sm:text-[13px] italic ml-0 sm:ml-9 mb-1.5" style={{ color: '#64748b', lineHeight: 1.6, margin: q.answer ? '0 0 6px' : 0 }}>"{q.answer}"</p>}
              {q.evaluation && <p className="text-xs sm:text-[13px] ml-0 sm:ml-9 p-2 sm:p-[8px_12px] rounded-lg" style={{ color: '#475569', lineHeight: 1.5, background: '#f8fafc', borderLeft: '3px solid #06b6d4', margin: 0 }}>{q.evaluation}</p>}
            </div>
          ))}
        </div>
      )}
...
const HistoryList = () => {
  ...
  return (
    <div>
      <h1 className="text-xl sm:text-2xl font-bold mb-4 sm:mb-6" style={{ fontFamily: 'Syne, sans-serif', color: '#1e293b' }}>Interview History</h1>
      {sessions.length === 0 ? (
        <div className="p-10 sm:p-[60px] text-center" style={{ background: 'white', borderRadius: 16, boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
        ...
        <div className="flex flex-col gap-2 sm:gap-3">
          {sessions.map(s => (
            <div key={s._id} onClick={() => navigate(`/history/${s._id}`)}
              ...
              style={{ background: 'white', borderRadius: 16, boxShadow: '0 1px 3px rgba(0,0,0,0.06)', cursor: 'pointer', transition: 'box-shadow 0.15s' }}
              onMouseEnter={e => e.currentTarget.style.boxShadow = '0 4px 20px rgba(0,0,0,0.1)'}
              onMouseLeave={e => e.currentTarget.style.boxShadow = '0 1px 3px rgba(0,0,0,0.06)'}
            >
              <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-xl flex items-center justify-center text-lg sm:text-xl shrink-0" style={{ background: '#f1f5f9' }}>💼</div>
              <div className="flex-1 min-w-0">
                <div className="text-sm sm:text-[15px] font-bold mb-1 truncate" style={{ color: '#1e293b' }}>{s.role}</div>
        ...
        <div className="flex justify-center gap-2 mt-5">
          {Array.from({ length: pagination.pages }, (_, i) => (
            <button key={i} onClick={() => setPage(i + 1)}
              ...
              style={{ border: `1.5px solid ${page === i + 1 ? '#06b6d4' : '#e2e8f0'}`, background: page === i + 1 ? '#06b6d4' : 'white', color: page === i + 1 ? 'white' : '#475569', cursor: 'pointer', fontFamily: 'inherit' }}
            >
              {i + 1}
            </button>
          ))}
